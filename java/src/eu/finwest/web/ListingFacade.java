...
public class ListingFacade {
	...
	public ListingAndUserVO updateListingProperties(UserVO loggedInUser, String listingId, List<ListingPropertyVO> properties) {
		...
		if (listing.state != Listing.State.NEW) {
			...
			if (propsToUpdate.isEmpty()) {
				result.setErrorCode(ErrorCodes.OPERATION_NOT_ALLOWED);
				result.setErrorMessage(Translations.getText("lang_error_listing_not_new"));
				return result;
			}
		...
		for (ListingPropertyVO prop : properties) {
			...
			if (ListingVO.UPDATABLE_PROPERTIES.contains(propertyName)) {
                ...
                if (doc == null) {
                    ...
                    result.setErrorMessage(Translations.getText("lang_error_listingdoc_error"));
                    return result;
                }
		...
		if (propsToUpdate.isEmpty() && !fetchedDoc) {
			...
			result.setErrorMessage(Translations.getText("lang_error_listing_not_updated"));
			return result;
		}
	...
	public ListingAndUserVO updateListingAddressProperties(UserVO loggedInUser, Map<String, String> properties) {
		...
		if (infos.length() > 0 ) {
			...
			result.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			result.setErrorMessage(Translations.getText("lang_error_listing_edited_missing_address"));
			return result;
		}
	...
	public ListingAndUserVO activateListing(UserVO loggedInUser, String listingId) {
		...
		if (loggedInUser == null) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_user_not_logged_in"));
			return returnValue;
		}
		...
		if (!loggedInUser.isAdmin()) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_user_not_admin"));
			return returnValue;
		}
		...
		if (dbListing.state != Listing.State.POSTED && dbListing.state != Listing.State.FROZEN) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_listing_not_posted_or_frozen"));
			return returnValue;
		}
	 ...
	 * Makes listing ready to be published on InwestujWFirmy website, but t
	 */
	public ListingAndUserVO postListing(UserVO loggedInUser, String listingId) {
		...
		if (!StringUtils.equals(loggedInUser.getId(), dbListing.owner.getString())) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_user_not_project_owner"));
		...
		if (FrontController.getCampaign() != null && !FrontController.getCampaign().getSubdomain().equals(dbListing.campaign)) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_listing_for_different_campaign"));
			returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			return returnValue;
		}
...
//			}
		...
		returnValue.setErrorMessage(Translations.getText("lang_error_listing_not_new"));
		returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
		return returnValue;
	}
...
//		checkMandatoryStringField(logs, "Answer26", listing.answer26, 16, 512);
	...
	public ListingAndUserVO withdrawListing(UserVO loggedInUser, String listingId) {
		...
		if (dbListing == null) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_listing_not_found"));
			returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			return returnValue;
		}
		if (!StringUtils.equals(loggedInUser.getId(), dbListing.owner.getString())) {
		...
		if (dbListing.state == Listing.State.ACTIVE) {
			...
			if (updatedListing != null) {
				...
				returnValue.setErrorMessage(Translations.getText("lang_error_listing_not_updated"));
		...
		returnValue.setErrorMessage(Translations.getText("lang_error_listing_not_allowed_for_closed_or_withdrawn"));
		returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
		return returnValue;
	}

	/**
	 * Sends back listing for update to the owner as it is not ready for posting on inwestuj-w-firmy site
	 */
	public ListingAndUserVO sendBackListingToOwner(UserVO loggedInUser, String listingId, String message) {
		...
		if (user == null) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_user_not_found"));
			returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			return returnValue;
		}
		if (dbListing.state == Listing.State.POSTED || dbListing.state == Listing.State.FROZEN) {
		...
		returnValue.setErrorMessage(Translations.getText("lang_error_listing_cannot_be_sent_back"));
		returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
		return returnValue;
	}
	...
	public ListingAndUserVO freezeListing(UserVO loggedInUser, String listingId, String message) {
		...
		if (dbListing == null || dbListing.state == Listing.State.NEW || dbListing.state == Listing.State.POSTED) {
			...
			returnValue.setErrorMessage(Translations.getText("lang_error_listing_doesnt_exist"));
			returnValue.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			return returnValue;
		}
	...
	public ListingAndUserVO deleteEditedListing(UserVO loggedInUser, String listingId) {
		...
		if (loggedInUser == null) {
			...
			result.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			result.setErrorMessage(Translations.getText("lang_error_listing_delete_not_owner"));
			return result;
		}
		...
		if (deletedListing == null) {
			...
			result.setErrorCode(ErrorCodes.DATASTORE_ERROR);
			result.setErrorMessage(Translations.getText("lang_error_listing_not_deleted"));
			return result;
		}
	...
	public ListingListVO getMonitoredListings(UserVO loggedInUser, String userId, ListPropertiesVO listProperties) {
		...
		if (loggedInUser.isAdmin() && StringUtils.isNotEmpty(userId)) {
			...
			if (user == null) {
				...
				list.setErrorMessage(Translations.getText("lang_error_user_not_found"));
				return list;
			}
		}
...
//		list.setListings(listings);
	...
	public ListingListVO getUserListings(UserVO loggedInUser, String stateString, String userId, ListPropertiesVO listingProperties) {
		...
		if (!(isAdmin || ownerListings) && state != Listing.State.ACTIVE) {
			...
			list.setErrorMessage(Translations.getText("lang_error_user_not_admin"));
	...
	public ListingDocumentVO createListingDocument(UserVO loggedInUser, String listingId, ListingDocumentVO doc) {
		...
		if (loggedInUser == null) {
			...
			doc.setErrorMessage(Translations.getText("lang_error_user_not_logged_in"));
			log.info("User not logged in!");
			return doc;
		}
		...
		if (!(loggedInUser.isAdmin() || StringUtils.equals(loggedInUser.getId(), listing.owner.getString()))) {
			blobstoreService.delete(doc.getBlob());
			doc.setErrorCode(ErrorCodes.OPERATION_NOT_ALLOWED);
			doc.setErrorMessage(Translations.getText("lang_error_user_not_project_owner"));
			return doc;
		}
...
//		}
		if (listing.state == Listing.State.CLOSED || listing.state == Listing.State.WITHDRAWN) {
			result.setErrorCode(ErrorCodes.OPERATION_NOT_ALLOWED);
			result.setErrorMessage(Translations.getText("lang_error_listing_not_allowed_for_closed_or_withdrawn"));
			return result;
		}
	...
	public ListingContributionsVO createContribution(UserVO loggedInUser, ContributionVO contrib) {
		...
		contribution.money = NumberUtils.createDouble(contrib.getMoney().replace(",", "."));
		...
		ServiceFacade.instance().setListingMonitor(loggedInUser, dbListing.getWebKey());
	...
	public ListingContributionsVO deleteContribution(UserVO loggedInUser, String contributionId) {
		...
		Listing dbListing = getDAO().getListing(contrib.listing.getString());
		if (!StringUtils.equals(loggedInUser.getId(), dbListing.owner.getString())
				&& !StringUtils.equals(loggedInUser.getId(), contrib.contributor.getString())) {
			log.log(Level.INFO, "User '" + loggedInUser + "' is not an owner of listing " + dbListing);
			result.setErrorMessage(Translations.getText("lang_error_user_not_project_owner"));
			result.setErrorCode(ErrorCodes.NOT_AN_OWNER);
			return result;
	...
	public ListingContributionsVO removeContributor(UserVO loggedInUser, String listingId, String userId) {
		...
		if (dbListing == null) {
			log.log(Level.INFO, "Listing doesn't exist.");
			result.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			result.setErrorMessage(Translations.getText("lang_error_listing_doesnt_exist"));
			return result;
		}
		if (!loggedInUser.isAdmin() && !StringUtils.equals(loggedInUser.getId(), dbListing.owner.getString())) {
			log.log(Level.INFO, "User '" + loggedInUser + "' is not an owner of listing " + dbListing);
			result.setErrorMessage(Translations.getText("lang_error_user_not_project_owner"));
			result.setErrorCode(ErrorCodes.NOT_AN_OWNER);
			return result;
		}
		if (StringUtils.equals(userId, dbListing.owner.getString())) {
			log.log(Level.INFO, "Cannot remove owner of listing from project contributors.");
			result.setErrorMessage(Translations.getText("lang_error_contr_user_is_owner"));
			result.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
			return result;
		}
		
		prepareContributionsData(loggedInUser, dbListing, result);
		for (UserContributionVO contrib : result.getTotalContributions()) {
			if (StringUtils.equals(contrib.getContributor(), userId)) {
				if (!contrib.isZero()) {
					log.log(Level.INFO, "User '" + userId + "' has already registered contributions, " + contrib);
					result.setErrorMessage(Translations.getText("lang_error_contr_user_has_contributions",
							contrib.getContributorName(), dbListing.name));
					result.setErrorCode(ErrorCodes.ENTITY_VALIDATION);
					return result;
				}
			}
		...
		if (StringUtils.contains(dbListing.contributors, userId)) {
			String[] contributors = StringUtils.split(dbListing.contributors, " ");
			dbListing.contributors = "";
			for (String contrId : contributors) {
				if (StringUtils.equals(contrId, userId)) {
					continue;
				}
				if (StringUtils.isBlank(dbListing.contributors)) {
					dbListing.contributors = contrId;
				} else {
					dbListing.contributors += " " + contrId;
				}
			}
			...
			MemCacheFacade.instance().updateListingContributors(userId);
			prepareContributionsData(loggedInUser, dbListing, result);
		}
